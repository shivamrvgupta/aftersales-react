...
import { OtherList } from "../../../helpers/url_helper";
...
const AdminRolesList = () => {
  ...
  const breadcrumbItems = [
    { title: "Users", link: "/users/all-customers" },
    { title: "All Customer", link: "/users/all-customers" },
  ];

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await get(OtherList);
        setData(response.data.users);
        console.log("Other Users Data ---", response);
      } catch (error) {
        setError(error);
      } finally {
        setLoading(false);
  ...
  const allOtherUsers = data.slice(
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Other User's Lists" breadcrumbItems={breadcrumbItems} />
          <Card>
            <CardBody>
                    ...
                    <table>
                      ...
                      <tbody>
                        {allOtherUsers.length > 0 ? (
                          allOtherUsers.map((customer, index) => (
            ...
            <CardFooter className="table-card-footer">
              <div className="pagination">
                <p>
                  Showing {allOtherUsers.length > 0
