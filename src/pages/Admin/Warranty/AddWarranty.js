...
const AddProduct = () => {
  ...
  const breadcrumbItems = [
    { title: "Warranty", link: "/warranty/all-warranties-list" },
  ...
  const handleFormRegister = async (e) => {
    ...
    const formData = {
      ...
      customer_id: selectedCustomer, // Include the selected customer ID
    ...
    try {
      ...
      const response = await post("/admin/warranty/add-warranty", formData, {
        headers: {
      ...
      if (response) {
        setTimeout(() => {
          ...
          setTimeout(() => {
            navigate("/warranty/all-warranties-list");
          }, 5000);
            ...
            <Card className="Add-WarrantyForm">
              <CardBody>
                ...
                {formSubmitted && selectedProduct && (
                  <>
                    <div className="input-container" style={{ marginTop: "1%" }}>
                      ...
                      >
                        ...
                        {customerData.map((customer, index) => (
                          <option key={index} value={customer.user_id}>
                            {customer.name} - {customer.phone_number} {/* Updated display */}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="input-container" style={{ marginTop: "1%" }}>
                      <label htmlFor="attachment">
                        Attachment (Bill Copy / Invoice / Purchase Proof)
                      </label>
                      <input
                        className="form-control"
                        name="attachment"
